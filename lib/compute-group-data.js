...
function computeGroupData(data, staff) {
  ...
  cgData.created = new Date(data[0]["creation-date"] || data[0]["start-date"] || ((data[4][0] || {}).created + "Z"));
  ...
  if (data[3] && data[3].length) {
    ...
    data[3].forEach(({service, data, error}) => {
      let perMonthData;
      if (data && data.items) {
	...
	if (cgData.activity[service.type]) {
	  if (!cgData.activity[service.type].errors) {
	    cgData.activity[service.type].errors = [error];
	  }
	  cgData.activity[service.type].errors.push(error);
	} else {
	  cgData.activity[service.type] = {errors: [error]};
	}
      }
