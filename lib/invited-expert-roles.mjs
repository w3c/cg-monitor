	...
	.map(async p =>
	  {
	    ...
	    let affiliations = [];
	    try {
	      affiliations = (await recursiveW3cFetch(`https://api.w3.org/users/${id}/affiliations?embed=1`, 'affiliations') || []).filter(a => a._links.self.href !== "https://api.w3.org/affiliations/36747").map(a => { return { name: a.name, homepage: a._links.homepage?.href, href: a._links.self.href}; });
	    } catch (e) {
	      console.error(e);
	    }
	    return {
	      ...
	      photo: p._links.photos?.find(p => p.name === "thumbnail")?.href,
	      created: p.created,
