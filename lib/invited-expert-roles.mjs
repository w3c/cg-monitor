...
for(let g of groups) {
  const participants = (await w3c.group(g.fullshortname).users().fetch({embed:true})) || [];
  // There are IGs for which this doesn't work, but they don't have IEs by definition
  let participations = [];
  try {
    participations = (await w3c.group(g.fullshortname).participations().fetch({embed: true})) || [];
  } catch (e) {
    continue;
  }
  const chairs = (await w3c.group(g.fullshortname).chairs().fetch()) || [];
  const ies = participations.filter(p => p["invited-expert"])
	.map(p =>
	  { return {
